...
public class SMBSettingsActivity extends Activity {
	...
	private final void performSelect() {
		...
		// build stores task
		EnumerateStoresTask task = new EnumerateStoresTask(SMBSettingsActivity.this);
	...
	private void initializeConfigurationDialog(final Dialog view) {
		...
		String user_name = settings.getString(ConfigurationSettings.SMB_USER_NAME, "");
		String pwd = settings.getString(ConfigurationSettings.SMB_PASSWORD, "");
		String server = settings.getString(ConfigurationSettings.SMB_SERVER, "");
		String share = settings.getString(ConfigurationSettings.SMB_SHARE, "");
		
		// set credentials
		final EditText user_name_txt = (EditText)view.findViewById(R.id.user_name_value);
		if (user_name_txt != null)
			user_name_txt.setText(user_name);
		
		final EditText pwd_txt = (EditText)view.findViewById(R.id.password_value);
		if (pwd_txt != null)
			pwd_txt.setText(pwd);

		final EditText server_txt = (EditText)view.findViewById(R.id.server_value);
		if (server_txt != null)
			server_txt.setText(server);
		
		final EditText share_txt = (EditText)view.findViewById(R.id.share_value);
		if (share_txt != null)
				...
				public void onClick(View v) {
					...
					if (user_name_txt != null) {
						...
						if (user_name_txt.getText().toString().length() == 0) {
							ToastManager.getInstance().displayToastWithString(R.string.error_no_user_name);
							return;
						}
					}
					...
					if (pwd_txt != null) {
						...
						if (pwd_txt.getText().toString().length() == 0) {
							ToastManager.getInstance().displayToastWithString(R.string.error_no_password);
							return;
						}
					}
					...
					if (server_txt != null) {
						...
						if (server_txt.getText().toString().length() == 0) {
							ToastManager.getInstance().displayToastWithString(R.string.error_no_server);
							return;
						}
					}
					...
					if (share_txt != null) {
						...
						if (share_txt.getText().toString().length() == 0) {
							ToastManager.getInstance().displayToastWithString(R.string.error_no_share);
							return;
						}
					}
					...
					if (connect)
					{
						...
						ToastManager.getInstance().displayToastWithString(R.string.connect_success);
					...
					{
						// failed to connect
						ToastManager.getInstance().displayToastWithString(R.string.error_failed_connect);
					}
				}
	...
	private void saveConfiguration(String username, String password, String server, String share) {
	...
	private boolean connect(String username, String password, String server, String share) {
	...
	private final void handleSelection(CharSequence tagstore_name) {
		...
		// store new tagstore
		editor.putString(ConfigurationSettings.CURRENT_SYNCHRONIZATION_TAGSTORE, (String)path);
