...
public class DropboxSynchronizerActivity extends Activity implements
		SynchronizationAlgorithmCallback, SyncTaskCallback {
	 ...
	 * // authenticate try {
	 * 
	 * Logger.e("logging in"); boolean login = provider.login("Johnseyii",
	 * "1234", "192.168.2.2"); Logger.e("logged in: " + login);
	 * 
	 * if (!login) return false;
	 * 
	 * ArrayList<String> files = provider.getFiles("/", null); for(String file :
	 * files) Logger.e(file);
	 * 
	 * Logger.e("done");
	 * 
	 * // acquire shared settings SharedPreferences settings =
	 * getSharedPreferences( ConfigurationSettings.TAGSTORE_PREFERENCES_NAME,
	 * Context.MODE_PRIVATE);
	 * 
	 * SharedPreferences.Editor editor = settings.edit();
	 * 
	 * editor.putString(ConfigurationSettings.CURRENT_SYNCHRONIZATION_TAGSTORE,
	 * "/tagstore/store1"); editor.commit(); return true;
	 * 
	...
	private boolean initializeBackend() {
		...
		StorageProviderFactory provider_factory = new StorageProviderFactory();
		provider_factory
				.initializeStorageProviderFactory(DropboxSynchronizerActivity.this);
		...
		String target_store = settings.getString(
	...
	private void initializeUI() {
		...
		// read target tagstore
		m_target_store = settings.getString(
				ConfigurationSettings.CURRENT_SYNCHRONIZATION_TAGSTORE, "");
	...
	private void performSynchronization() {
		...
		Thread sync_thread = new Thread(new Runnable() {
			...
			public void run() {
				...
				if (m_backend.prepareSynchronization())
					if (m_backend.performSynchronization())
	...
	private void showConflictDialog(ConflictData data) {
		...
		AlertDialog alert_dialog = new AlertDialog.Builder(
				DropboxSynchronizerActivity.this).create();
	...
	public void notifyFileSyncInfo(final int source_files,
			final int target_files) {
		runOnUiThread(new Runnable() {

			public void run() {

				// source store files + target store files + 2 * (target store
				// file + target sync store file)
