...
public class TagStackUIButtonAdapter extends RelativeLayout {
	...
	/**
	 * sets an event dispatcher interface
	 * 
	 * @param event
	 */
	public void setEventDispatcher(EventDispatcherInterface event) {
		m_event = event;
	}
	...
	private void init() {
		...
		for (int index = 0; index < 5; index++) {
			...
			// new layout param
			//
			LayoutParams layout_params = new RelativeLayout.LayoutParams(
			...
			if (index == 0) {
				...
				//
				layout_params.addRule(RelativeLayout.RIGHT_OF,
						m_buttons[index - 1].getId());
			}
	...
	private class ButtonClickListener implements OnClickListener {
		...
		public void onClick(View arg0) {
			...
			// signal event
			//
			m_event.signalEvent(EventDispatcher.EventId.TAG_STACK_BUTTON_EVENT,
					event_args);
		}
	};
	...
	private class ButtonLongClickListener implements OnLongClickListener {
		...
		public boolean onLongClick(View arg0) {
			...
			//
			m_event.signalEvent(
					EventDispatcher.EventId.TAG_STACK_LONG_BUTTON_EVENT,
					event_args);
